...
| Usar la mano con servo + ordenador | `mano_robotica` | Ordenador |
| Usar la mano con servo sin ordenador | `mano_autonoma` | Calibrar antes y copiar valores |
| Usar la mano con servo + botón | `mano_boton` | Pulsador en pin 2 |
| **--- Motor paso a paso 28BYJ-48 ---** | | |
| Calibrar pasos del stepper | `calculo_pasos_stepper` | Ordenador |
| Usar la mano con stepper + ordenador | `mano_robotica_stepper` | Ordenador |
...
### 3.5 Motor paso a paso 28BYJ-48 + ULN2003

**IMPORTANTE: NO alimentar el motor desde el 5V del Arduino.** El 28BYJ-48 consume ~240 mA por fase, demasiado para el Arduino. Usar siempre fuente externa de 5V. Comportamiento inconsistente y mal rendimiento si se alimenta desde el Arduino.

| Pin ULN2003 | Conectar a |
|---|---|
| IN1 | Pin 8 Arduino |
| IN2 | Pin 9 Arduino |
| IN3 | Pin 10 Arduino |
| IN4 | Pin 11 Arduino |
| GND | GND común (Arduino + fuente externa) |
| VCC | Fuente externa 5V (NO el 5V del Arduino) |
| Motor connector | 28BYJ-48 (conector blanco, solo encaja en una posición) |
...
Fuente 5V ----------- ULN2003 VCC
```

Notas:
- El jumper ON/OFF del módulo ULN2003 debe estar puesto (ON)
- Los 4 LEDs del módulo indican qué bobina está activa — solo son indicadores
- El conector del motor solo encaja en una posición — no hay riesgo de conectarlo al revés
...
### 6.8 Calibración de pasos del stepper (28BYJ-48)

Programa interactivo para encontrar el número exacto de pasos que necesita el motor para cerrar la pinza completamente. Comandos por Monitor Serie:
- `c` → cierra (mueve al valor estimado + ajuste)
- `a` → abre (vuelve a posición 0)
- `+` → añade 20 pasos al ajuste
- `-` → quita 20 pasos al ajuste
- `k` → guarda el valor final de pasos máximos

El valor que se muestra al pulsar `k` es el `PASOS_MAXIMOS` que hay que copiar al programa de la mano.

### 6.9 Mano robótica con stepper (28BYJ-48)

Programa principal para controlar la mano con motor paso a paso. Misma lógica que el programa del servo pero adaptado al stepper:
- Control por pasos en lugar de microsegundos — posición exacta sin punto muerto
- El motor se mueve de forma no bloqueante (usa `millis()` en lugar de `delay()`)
- El estado 2 (CERRANDO) espera a que el motor llegue a la posición objetivo en lugar de usar un temporizador
- Requiere calibrar previamente los pasos con `calculo_pasos_stepper`

Ventajas respecto al servo DS04-NFC:
- Para exacto sin energía residual
- No necesita calibrar punto muerto
- Posición precisa y repetible

### 6.10 Mano robótica — Modo autónomo (sin ordenador)
...
| 28BYJ-48 vibra pero no gira | Programa con secuencia de bobinas incorrecta | Usar secuencia wave drive (1 bobina cada vez) con control manual |
| 28BYJ-48 solo gira en un sentido | Programa no invierte correctamente la secuencia | Usar pasoActual++ y pasoActual-- con variable dirección |
| 28BYJ-48 no se mueve | Alimentado desde 5V Arduino (corriente insuficiente) | Usar fuente externa de 5V |
| 28BYJ-48 se calienta | Bobinas activas sin movimiento | Llamar pararMotor() para apagar bobinas al parar |
...
### 8.3 28BYJ-48 + ULN2003

- NO alimentar desde el 5V del Arduino — usar fuente externa de 5V
- GND de Arduino y fuente externa DEBEN estar conectados entre sí
- El motor se calienta si las bobinas están activas mucho tiempo — apagar bobinas al parar
- Secuencia wave drive (1 bobina): menor torque, menor consumo
- Secuencia full step (2 bobinas): máximo torque, máximo consumo
- 2048 pasos por vuelta en full step, 4096 en half step
- Velocidad máxima ~15 RPM — más lento que un servo
- El jumper ON/OFF del ULN2003 debe estar puesto
- Los 4 LEDs del módulo solo son indicadores de actividad de bobinas
- La librería Stepper.h puede dar problemas con algunos Arduino — el control manual de bobinas es más fiable

### 8.4 Arduino UNO R4 WiFi
...
### 8.5 Control proporcional (futuro)
...
- Firmware del ESP32-S3 actualizado — en el IDE ir a Herramientas → Firmware Updater → seleccionar UNO R4 WiFi → Check Updates
